...
class ExpertAgainstGaussianActor(tf.keras.Model):
    LOG_STD_CAP_MAX = 2  # np.e**2 = 7.389
    LOG_STD_CAP_MIN = -20  # np.e**-10 = 4.540e-05
    EPS = 1e-6

    def __init__(self, state_shape, action_dim, max_action, expert_model, uncertainty='ensemble', squash=False, name='gaussian_policy'):
        super().__init__(name=name)
    ...
    def call(self, states, test=False):
        ...
        expert_dist = self._expert_policy(states)
        expert_actions = expert_dist.mean() * self._max_action
        kl = tfp.distributions.kl_divergence(dist, expert_dist)

        return actions, log_pis, entropy, expert_actions, kl
...
class ExpertAgainstGaussianActor3(tf.keras.Model):
    ...
    def call(self, states, test=False):
        ...
        if test:
            ...
            kl = tfp.distributions.kl_divergence(dist, expert_dist)
            expert_actions = expert_dist.mean() * self._max_action
            return actions, log_pis, entropy, std, exp_log_pis, kl, expert_actions
