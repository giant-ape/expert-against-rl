...
from tf2rl.policies.tfp_gaussian_actor import ExpertAgainstGaussianActor3
...
class ExpertAgainst3(OffPolicyAgent):
    def __init__(
            self,
            state_shape,
            action_dim,
    ...
    def _setup_actor(self, state_shape, action_dim, lr, max_action=1.0):
        self.actor = ExpertAgainstGaussianActor3(state_shape, action_dim, max_action, self._expert_prior, self._uncertainty)
    ...
    def _get_action_body(self, state, test):
        actions, log_pis, entropy, std, expert_log_pis, kl, expert_actions = self.actor(state, test)
    ...
    def _train_body(self, states, actions, next_states, rewards, dones, weights):
        with tf.device(self.device):
            ...
            with tf.GradientTape(persistent=True) as tape:
                ...
                sample_actions, logp, entropy, std, _, kl, expert_actions = self.actor(states)  # Resample actions to update V
                ...
                if self.auto_alpha:
                    ...
                    # beta_loss = tf.reduce_mean((self.beta * tf.stop_gradient(current_min_q - expert_current_min_q)))
    ...
    def _compute_td_error_body(self, states, actions, next_states, rewards, dones):
        with tf.device(self.device):
            ...
            sample_actions, logp, entropy, std, expert_logp, kl, expert_actions = self.actor(states)
            kl_errors = kl
