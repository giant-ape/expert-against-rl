...
from tf2rl.policies.tfp_gaussian_actor import ExpertAgainstGaussianActor
...
class ExpertAgainst2(OffPolicyAgent):
    def __init__(
            self,
            state_shape,
            action_dim,
        ...
        if auto_alpha:
            ...
            self.log_beta = tf.Variable(tf.math.log(beta), dtype=tf.float32)
            self.beta = tfp.util.DeferredTensor(pretransformed_input=self.log_beta, transform_fn=tf.exp, dtype=tf.float32)
            self.beta_optimizer = tf.keras.optimizers.Adam(learning_rate=lr, beta_1=0.5)
        else:
            self.alpha = alpha
    ...
    def _setup_actor(self, state_shape, action_dim, lr, max_action=1.):
        self.actor = ExpertAgainstGaussianActor(state_shape, action_dim, max_action, self._expert_prior, self._uncertainty, squash=True)
    ...
    def _get_action_body(self, state, test):
        actions, log_pis, entropy, expert_actions, kl = self.actor(state, test)

        return actions
    
    @tf.function
    def _get_action_value(self, state, action, test):
        current_q1 = self.qf1(state, action)
        current_q2 = self.qf2(state, action)
        current_min_q = tf.minimum(current_q1, current_q2)

        _, _, _, expert_actions, _ = self.actor(state, test)
        # expert_actions = np.expand_dims(expert_actions, axis=0).astype(np.float32) if is_single_state else state
        expert_current_q1 = self.qf1(state, expert_actions)
        expert_current_q2 = self.qf2(state, expert_actions)
        expert_current_min_q = tf.minimum(expert_current_q1, expert_current_q2)
        return current_min_q, expert_current_min_q

    def get_action_value(self, state, action, test=False):
        assert isinstance(state, np.ndarray)
        is_single_state = len(state.shape) == self.state_ndim

        state = np.expand_dims(state, axis=0).astype(np.float32) if is_single_state else state
        action = np.expand_dims(action, axis=0).astype(np.float32) if is_single_state else state
        current_min_q, expert_current_min_q = self._get_action_value(tf.constant(state), tf.constant(action), test)

        return current_min_q.numpy()[0],expert_current_min_q.numpy()[0]
    ...
    def _train_body(self, states, actions, next_states, rewards, dones, weights):
        with tf.device(self.device):
            ...
            with tf.GradientTape(persistent=True) as tape:
                ...
                sample_actions, logp, entropy, expert_actions, kl = self.actor(states)  
                current_q1 = self.qf1(states, sample_actions)
                current_q2 = self.qf2(states, sample_actions)
                current_min_q = tf.minimum(current_q1, current_q2)

                expert_current_q1 = self.qf1(states, expert_actions)
                expert_current_q2 = self.qf2(states, expert_actions)
                expert_current_min_q = tf.minimum(expert_current_q1, expert_current_q2)
                ...
                # policy_loss = tf.reduce_mean(self.alpha * logp - (current_min_q - expert_current_min_q)*5)
                # advantage = current_min_q - expert_current_min_q
                advantage = tf.minimum(current_q1 - expert_current_q1, current_q2 - expert_current_q2)

                # # policy_loss = tf.reduce_mean(self.alpha * logp 
                # #                              - self.beta * advantage
                # #                              - current_min_q)
                # policy_loss = tf.reduce_mean(self.alpha * (kl - self.epsilon) 
                #                              - self.beta * advantage
                #                              - current_min_q)
                ...
                #     beta_loss = -tf.reduce_mean((self.beta * tf.stop_gradient(expert_current_min_q - current_min_q)))
                ...
                if self.step < 20000:
                    ...
                    policy_loss = tf.reduce_mean(- self.beta * advantage
                                                - current_min_q) 
                    beta_loss = tf.reduce_mean(self.beta * tf.stop_gradient(advantage))
            ...
            if self.step < 20000:
                ...
                beta_grad = tape.gradient(beta_loss, [self.log_beta])
                self.beta_optimizer.apply_gradients(zip(beta_grad, [self.log_beta]))    
